...
tf.app.flags.DEFINE_string('data_dir', '/workspace/planet-understanding-the-amazon-from-space',
...
def inference(images):
    ...
    with tf.variable_scope('conv1') as scope:
        kernel = _variable_with_weight_decay('weights',
                                             shape=[5, 5, 4, 64],  # 4 channels (RGBIr)
                                             stddev=5e-2,
                                             wd=None)
...
def loss(logits, labels):
    ...
    cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(  # using sigmoid because we need classes to be ranked independently and softmax is not.
